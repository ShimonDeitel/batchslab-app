import XCTest
@testable import BatchSlab

@MainActor
final class BatchSlabTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntry() {
        store.entries = []
        store.add(name: "Test Item", wood: "x", dimensions: "x", resin: "x", stage: "x")
    ...
    func testCannotAddMoreAtLimitWhenNotPro() {
        ...
        store.entries = (0..<Store.freeLimit).map { i in
            SlabEntry(name: "Item \(i)", wood: "", dimensions: "", resin: "", stage: "")
        }
    ...
    func testDeleteEntry() {
        store.entries = []
        store.add(name: "ToDelete", wood: "x", dimensions: "x", resin: "x", stage: "x")
    ...
    func testUpdateEntry() {
        store.entries = []
        store.add(name: "Original", wood: "x", dimensions: "x", resin: "x", stage: "x")
